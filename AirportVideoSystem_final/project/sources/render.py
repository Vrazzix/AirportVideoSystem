...
class Camera:
    ...
    def update(self, rotation_center=(0, 0, 0)):
        # Вычисляем смещение камеры относительно центра вращения
        x_offset = self.orbital_radius * np.cos(self.polar) * np.cos(self.azimuth)
        y_offset = self.orbital_radius * np.sin(self.polar)
        z_offset = self.orbital_radius * np.cos(self.polar) * np.sin(self.azimuth)

        # Преобразуем rotation_center в плоский список float, чтобы избежать проблем с формами матриц (3,1) vs (3,)
        # Это гарантирует, что мы работаем с обычными числами при сложении
        try:
            # Если rotation_center - это numpy массив (любой формы), сплющиваем его и берем элементы
            cx = float(np.asarray(rotation_center[0]).flatten()[0])
            cy = float(np.asarray(rotation_center[1]).flatten()[0])
            cz = float(np.asarray(rotation_center[2]).flatten()[0])
        except (IndexError, TypeError, AttributeError):
            #Fallback на случай, если это уже обычный список/кортеж
            cx = float(rotation_center[0])
            cy = float(rotation_center[1])
            cz = float(rotation_center[2])

        # Теперь self.position будет кортежем из обычных float, а не массивов
        self.position = (x_offset + cx, y_offset + cy, z_offset + cz)

        glMatrixMode(GL_MODELVIEW)
        glLoadIdentity()
        
        # Явно извлекаем значения как float (на всякий случай, хотя выше мы уже это сделали)
        # Используем .item() если элемент вдруг остался массивом 0-мерным, но float() после нашей обработки должен сработать
        eye_x = float(self.position[0])
        eye_y = float(self.position[1])
        eye_z = float(self.position[2])
        ...
        gluLookAt(eye_x, eye_y, eye_z,      # from
...
class Renderer3D:
    ...
    def draw_cube(self, position=(0.0, 0.0, 0.0)):
        ...
        # Гарантированное получение скаляров из position 
        # (работает для tuple, list, np.array формы (3,) или (3,1))
        pos_array = np.asarray(position).flatten()
        px = float(pos_array[0])
        py = float(pos_array[1])
        pz = float(pos_array[2])

        for edge in edges:
            for vertex in edge:
                glVertex3f(vertices[vertex][0] + px,
