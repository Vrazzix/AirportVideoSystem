...
with st.sidebar:
    ...
    model_path_wheels = st.text_input("🛞 Комбо-модель (колёса+колодки)", value="C:\\Users\\shche\\Desktop\\Application_for_models\\models\\BestBoots_v2.pt")
    model_path_person = st.text_input("🧑 Модель людей", value="C:\\Users\\shche\\Desktop\\Application_for_models\\models\\person.pt")
    model_path_pose   = st.text_input("🦴 Модель позы (MPII 16)", value="C:\\Users\\shche\\Desktop\\Application_for_models\\models\\BestPose.pt")
    ...
    st.markdown("**🎭 Маскировка динамических объектов**")
    slam_visual_masking = st.checkbox(
        "YOLOv8-seg маска перед SIFT", value=True,
        help="DynamicMasker из slam.zip маскирует людей и машины "
             "до SIFT-детектирования — облако точек будет чище."
    )
    slam_yolo_model = st.text_input(
        "YOLO-seg модель", value="yolov8n-seg.pt",
        help="yolov8n-seg.pt (быстрее) или yolov8s-seg.pt (точнее)"
    )
    slam_yolo_conf = st.slider("Conf маскировщика", 0.1, 0.8, 0.25, 0.05)
...
if uploaded_file is not None:
    ...
    if st.button("🚀 Запустить обработку", type="primary", use_container_width=True):
        ...
        if not loaded:
            ...
            slam    = (SemanticSLAM(
                           min_observations=slam_min_obs,
                           min_features=slam_min_feat,
                           enable_visual_masking=slam_visual_masking,
                           yolo_model=slam_yolo_model,
                           yolo_conf=slam_yolo_conf,
                       )
            ...
            if slam is not None:
                ...
                if mask_stats.get("enabled"):
                    st.info(
                        f"🎭 **Маскировка SIFT**: "
                        f"{mask_stats['filtered_kpts']:,} / {mask_stats['total_kpts']:,} "
                        f"ключевых точек отфильтровано "
                        f"({mask_stats['filter_rate_pct']}%) — "
                        f"динамические объекты исключены из облака точек."
                    )
